...
func main() {
	localIP := flag.String("LIP", "192.168.1.8", "Local IP that will listen for UDP replies")
	localAddr, err := net.ResolveUDPAddr("udp", *localIP+":11001")
	checkError(err)
...
func dnsReader(conn *net.UDPConn) {
	for {
		...
		if msg.Header.RCode == dnsmessage.RCodeSuccess {
			fmt.Printf("%+s %d ", msg.Questions[0].Name, msg.Header.ID)
			if len(msg.Answers) > 0 {
				fmt.Printf("%+v", msg.Answers[0].Body)
			} else {
				fmt.Printf(" -EEMPTYANS")
			}
			//fmt.Printf("%+v", msg)
			//fmt.Printf("%+v", msg.Answers[0].Body)
			continue
...
func dnsWriter(conn *net.UDPConn) {
	...
	fileHandle, _ := os.Open("/home/amit/nuWithAuthNS.Feed.Sample.txt")
	...
	for scanner.Scan() {
		...
		//if (i==64000) {return}
		go sendPacket(domain[0], domain[1], msg, conn)
	}
	fmt.Print("All Read : ")
	fmt.Printf(time.Now().Format("15:04:05.000000000") + "\n")
}

func sendPacket(domain string, nameserver string, msg dnsmessage.Message, conn *net.UDPConn) {
	...
	DNSServerAddr, err := net.ResolveUDPAddr("udp", nameserver+":53")
	checkError(err)
